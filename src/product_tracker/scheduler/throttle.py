"""Being a good citizen towards stores.

Two independent protections, both keyed by store:

* **Throttle** -- a minimum gap between requests to the same store, plus randomised
...
* **Circuit breaker** -- after a run of consecutive failures for one store, stop calling it
  for a cooling-off period. This is what stops a blocked or down store from being hammered,
  and it is per-store, so one failing site never delays products from any other.
...
class StoreGuard:
    ...
    def before(self, store_slug: str) -> GuardDecision:
        ...
        with self._lock:
            state = self._states.setdefault(store_slug, _StoreState())
            now = self._clock()
            ...
            if state.open_until is not None:
                if now < state.open_until:
                    ...
                    return GuardDecision.skip(
                        f"{store_slug} has failed {state.consecutive_failures} times in a row; "
                        f"backing off for another {remaining}s before trying again"
                    )
                # Cooling-off elapsed: half-open. Allow one probe through.
                state.open_until = None
                log.info("store.circuit_half_open", store=store_slug)
        ...
        with self._lock:
            ...
            if succeeded:
                if state.consecutive_failures:
                    log.info("store.recovered", store=store_slug)
    ...
    def snapshot(self) -> dict[str, dict[str, object]]:
        """Current state per store, for ``product-tracker status``."""
        with self._lock:
            now = self._clock()
            return {
                slug: {
