...
class Product(Base):
    ...
    #: Which worker is checking this right now, and since when. A lease rather than a
    #: lock, because the holder can die without releasing it -- see ``scheduler/claims``.
    #: Written by raw SQL so the claim is taken in the statement that tests it; mapped
    #: here so the model still describes the table.
    check_claimed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    check_claimed_by: Mapped[str | None] = mapped_column(String(64))
